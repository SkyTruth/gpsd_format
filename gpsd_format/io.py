...
def open(path, mode='r', dmode=None, cmode=None, compression=None, driver=None, do=None, co=None, **kwargs):
    ...
    """
    Return a collection instance that is set up to read or write with the
    specified driver.
    ...
    # The user didn't specify a compression type
    else:

        # Attempt to fetch the driver but if it fails just assume they didn't actually want compression
        try:
            compression_driver = drivers.detect_compression_type(path)
        except ValueError:
    ...
    if compression_driver is None:
        ...
        stream = driver(compression_driver.from_path(path, mode=cmode, **co), mode=dmode, **do)
